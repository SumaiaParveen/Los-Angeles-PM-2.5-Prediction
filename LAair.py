...
st.sidebar.markdown("### This application has been built based on a machine learning algorithm. All the input features have to be inserted in correct units (mentioned within parentheses beside each predictor) to get a result. The main pollutant of Los Angeles air is PM 2.5 and the objective of this app is to predict the concentration of PM 2.5 in micro-gram/cubic-meter. Mean Absolute Error is ~2.36 micro-gram/cubic-meter and R-squared value is 0.60")
...
# --------------------------------- joblib block ends --------------------------------
model = pickle.load(open('rf_PM25.pkl', 'rb'))  # get the model
